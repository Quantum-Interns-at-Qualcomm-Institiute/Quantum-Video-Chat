...
1. **Link-keyed MAC** (`channel-auth.js`): both sides derive per-direction HMAC keys (HKDF-SHA-256) from the invite link's capability token. Every classical message travels as `{seq, payload, tag}` with a monotonic per-direction sequence, so modification, injection, replay, reordering, and dropping all fail verification and abort the round as `auth-failure` — latched, no retry, red banner. The DTLS certificate fingerprints from the SDP are cross-checked over this authenticated channel before the first round, so a signaling server that substituted its own DTLS endpoints is caught unless it also holds the invite token.
2. **SAS display**: a short authentication string (6 digits + 4 emoji), derived from both DTLS fingerprints and the MAC transcript of the first completed round, is shown under the video on both sides with a "compare on camera" prompt and a mismatch button that tears the call down. Comparing it defeats even an attacker who obtained the invite link: a MITM cannot make both sides' transcripts hash to the same SAS.
