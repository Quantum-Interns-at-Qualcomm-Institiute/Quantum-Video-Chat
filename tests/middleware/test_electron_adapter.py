"""Tests for middleware/adapters/electron.py — ElectronSocketAdapter."""
import pytest
import sys
import os
...
_spec = importlib.util.spec_from_file_location(
    "middleware_adapters_electron",
    os.path.join(_middleware_dir, "adapters", "electron.py"))
_mod = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(_mod)
ElectronSocketAdapter = _mod.ElectronSocketAdapter

from shared.adapters import FrontendAdapter


class TestElectronSocketAdapter:
    ...
    def test_is_frontend_adapter(self):
        adapter = ElectronSocketAdapter(self._connected_socket())
    ...
    def test_send_frame_when_connected(self):
        ...
        adapter = ElectronSocketAdapter(mock_socket)
        data = b'\x00\x01\x02'
...
class TestSendStatus:
    """ElectronSocketAdapter.send_status emits correctly and is safe when disconnected."""

    def _make_adapter(self, connected: bool):
        mock_socket = MagicMock()
        mock_socket.connected = connected
        return ElectronSocketAdapter(mock_socket), mock_socket
